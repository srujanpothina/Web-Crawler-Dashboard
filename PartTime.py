...
class ParttimeSpider(scrapy.Spider):
    ...
    start_urls = (
        'https://www.glassdoor.com/Job/us-software-engineer-jobs-SRCH_IL.0,2_IN1_KO3,20.htm?fromAge=1&jobType=parttime',
    )
    ...
    def parse(self, response):
        ...
        if(self.count < 30):
            ...
            if (df.to_csv('JobList_PartTimeNew.csv', mode='a', index=False)):
                print("Success")
            ...
            for i in range(0,len(self.titles)):
                ...
                Values = [self.titles[i],"Part Time",self.companies[i],self.city[i],self.state[i],self.min_sal[i],self.max_sal[i],self.ur[i],self.skills[i]]
                cursor.execute(SQLCommand, Values)
                db.commit()
                cursor.execute("select max(id) from Jobs")
                rs = (cursor.fetchone())
                job_id=str(rs[0])
                print(str((job_id))+"--job id")
                my_list = self.skills[i].split(",")
                if (self.skills[i]!=""):
                    print("abc"+self.skills[i]+"abc")
                    if(len(my_list)>0):
                        print(len(my_list))
                        #cursor1 = db.cursor()
                        for j in my_list:
                            ...
                            query1 = "INSERT INTO [skill_mapping] values(?,?)"
                            skill_id=str(rs1[0])
                            vals=[int(skill_id),int(job_id)]
